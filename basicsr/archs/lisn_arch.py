...
# import torch
from basicsr.archs.basicblock import DRB, PALayer, CALayer, CCALayer, SRB, channel_shuffle
from basicsr.archs.SwinT import SwinT
...
class LISNModel(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x1 = torch.cat([r for r in retainV], 1).contiguous()
...
class HRBCT(nn.Module):
    def __init__(self, embed_dim=64, nf=64, num_heads=8,distillation_rate=0.50):
        ...
        self.Conv1_D2 = nn.Conv2d(int(self.remaining_channels * self.distillation_rate), int(self.remaining_channels * self.distillation_rate), 1)

        self.ST = SwinT(embed_dim=self.remaining_channels, heads=num_heads)



        self.SRB = SRB(int(nf*(1-distillation_rate)**2), int(nf*(1-distillation_rate)**2))
