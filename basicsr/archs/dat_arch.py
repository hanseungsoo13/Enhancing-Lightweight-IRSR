...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def __init__(
        ...
        self.dwconv = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim), nn.BatchNorm2d(dim), nn.GELU()
        )
    ...
    def calculate_mask(self, H, W):
        ...
        img_mask_0 = img_mask_0.view(
            1, H // self.split_size[0], self.split_size[0], W // self.split_size[1], self.split_size[1], 1
        )
        img_mask_0 = (
        ...
        img_mask_1 = img_mask_1.view(
            1, H // self.split_size[1], self.split_size[1], W // self.split_size[0], self.split_size[0], 1
        )
        img_mask_1 = (
    ...
    def forward(self, x, H, W):
        ...
        if (self.rg_idx % 2 == 0 and self.b_idx > 0 and (self.b_idx - 2) % 4 == 0) or (
            ...
            qkv_0 = torch.roll(
                qkv[:, :, :, :, : C // 2], shifts=(-self.shift_size[0], -self.shift_size[1]), dims=(2, 3)
            )
            qkv_0 = qkv_0.view(3, B, _L, C // 2)
            qkv_1 = torch.roll(
                qkv[:, :, :, :, C // 2 :], shifts=(-self.shift_size[1], -self.shift_size[0]), dims=(2, 3)
            )
    ...
    def __init__(self, dim, rank=2, seq_length=4, conv=False):
        ...
        if not conv:
            self.cpb_mlp = nn.Sequential(
                nn.Linear(self.rank, 512, bias=True), nn.ReLU(), nn.Linear(512, dim, bias=False)
            )
        else:
            self.cpb_mlp = nn.Sequential(
                nn.Conv1d(self.rank, 512, 1, bias=True), nn.ReLU(), nn.Conv1d(512, dim, 1, bias=False)
            )
        ...
        if not self.grid_exists:
            ...
            if self.rank == 1:
                ...
                relative_coords_table = (
                    torch.stack(torch.meshgrid([relative_coords_h, relative_coords_w])).contiguous().unsqueeze(0)
                )
...
class WindowAttention(nn.Module):
    def __init__(
        self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.0, proj_drop=0.0, resolution=0, seq_length=0
    ):
        super().__init__()
...
def window_partition(x, window_size):
    ...
    assert (
        H_padded % window_size == 0 and W_padded % window_size == 0
    ), "Padding failed to make H and W divisible by window_size"
...
class HAT(nn.Module):
    ...
    def forward(self, x, carrier_tokens):
        ...
        if self.sr_ratio > 1:
            ...
            ctr, x = x.split(
                [x.shape[1] - self.window_size * self.window_size, self.window_size * self.window_size], dim=1
            )
            ...
            if self.last and self.do_propagation:
                ...
                x = x + self.gamma1 * self.upsampler(ctr_image_space.to(dtype=torch.float32)).flatten(2).transpose(
                    1, 2
                ).to(dtype=x.dtype)
        return x, ct
...
class ResidualGroup(nn.Module):
    ...
    def __init__(
        ...
        self.hat_layer = HAT(dim=dim, num_heads=num_heads)

        self.global_tokenizer = TokenInitializer(dim, 224, 8, ct_size=1)
    ...
    def forward(self, x, x_size):
        ...
        x_ccnn = torch.cat((x, counterlet_features.to(x.device)), 1)
