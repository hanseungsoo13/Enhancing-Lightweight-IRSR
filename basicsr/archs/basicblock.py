...
def conv_dw(
    ...
    for t in mode:
        if t == "C":
            ...
            L.append(
                nn.Conv2d(
                    in_channels=in_channels, out_channels=out_channels, kernel_size=1, stride=1, padding=0, groups=1
                )
            )
        elif t == "T":
            L.append(
...
class SRB(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1,
        ...
        self.act = nn.SiLU(inplace=True) # SiLU
...
class PA(nn.Module):
    ...
    def __init__(self, nf):
        ...
        self.conv1d = nn.Conv2d(nf, nf, 1)
        self.conv3d = nn.Conv2d(nf, nf, 3, 1, 1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        
        y = self.conv1d(x)
        y = self.sigmoid(self.conv3d(y))
        out = torch.mul(x, y)
        out = self.conv3d(out)
...
class LISBlock(nn.Module):
    def __init__(self, embed_dim=64,
        ...
        self.Conv3_D1 = conv_dw(self.distilled_channels, self.distilled_channels, kernel_size, stride, padding, bias, mode, negative_slope)

        self.SRB = conv_dw(self.remaining_channels, self.remaining_channels, kernel_size, stride, padding, bias, mode, negative_slope)
        self.CCALayer = CCALayer(embed_dim)
    
    def forward(self,x):
        distilled_c2, remaining_c2 = torch.split(x, (self.distilled_channels, self.remaining_channels), dim=1)
        out1 = self.Conv3_D1(distilled_c2)
        out2 = self.SRB(remaining_c2)
        out = torch.cat([out1, out2], dim=1)
        out = self.CCALayer(out)
        out = out + x
        return out
...
class ConvMixer(nn.Module):
    def __init__(self, dim, kernel_size=3, patch_size=3):
        super().__init__()
...
class RCAGroup(nn.Module):
    def __init__(
        ...
        RG = [
            RCABlock(in_channels, out_channels, kernel_size, stride, padding, bias, mode, reduction, negative_slope)
            for _ in range(nb)
        ]
...
def downsample_maxpool(
    ...
    pool_tail = conv(
        in_channels, out_channels, kernel_size, stride, padding, bias, mode=mode[1:], negative_slope=negative_slope
    )
