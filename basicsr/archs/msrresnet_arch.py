...
class D_msrresnet0(nn.Module):
    def __init__(
        self, in_nc=3, out_nc=3, nc=128, nb=16, upscale=4, act_mode="r", conv_act_mode="r", upsample_mode="upconv"
    ):  # nc setting 32|64|128
        """
        ...
        assert "R" in act_mode or "L" in act_mode or "S" in act_mode, "Examples of activation function: R, L, BR, BL, IR, IL, S"
        ...
        if upscale == 3:
            m_uper = upsample_block(nc, nc, mode="3" + conv_act_mode)
        else:
            m_uper = [upsample_block(nc, nc, mode="2" + conv_act_mode) for _ in range(n_upscale)]

        H_conv0 = B.conv(nc, nc, mode="C" + conv_act_mode)
...
class En_MSRResNet0(nn.Module):
    def __init__(
        ...
        m_body = [B.ResBlock(nc, nc, mode='C'+act_mode+'C') for _ in range(nb)]
       #m_body = [B.LISBlock(nc, distillation_rate=0.50) for _ in range(nb)]
        m_body.append(B.conv(nc, nc, mode="C"))
